...
def plot_pipeline_results(baseline_history, postdef_history, defense_history, save_path):
    ...
    Args:
        baseline_history: baseline 攻击 per-step 指标列表（可为 None）
    ...
    # 检查是否有 baseline
    has_baseline = baseline_history is not None and len(baseline_history) > 0
    has_postdef = postdef_history is not None and len(postdef_history) > 0

    if has_baseline:
        steps_b = [m['step'] for m in baseline_history]
    if has_postdef:
        steps_p = [m['step'] for m in postdef_history]
    epochs_d = list(range(1, len(defense_history) + 1)) if defense_history else []
    has_defense = len(epochs_d) > 0
    ...
    ax = axes[1, 1]
    if defense_history and len(defense_history) > 0:
