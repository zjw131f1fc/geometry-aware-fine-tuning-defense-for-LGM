...
#
# 用法:
#   bash experiments/run_cross_dataset_generalization_naive.sh            # 默认 GPU=0 (单卡顺序执行)
#   bash experiments/run_cross_dataset_generalization_naive.sh 0          # 指定 GPU=0 (单卡顺序执行)
#   bash experiments/run_cross_dataset_generalization_naive.sh 0,1        # 多卡并行: 2张卡动态调度任务
...
CATEGORIES=(bowl box)  # 暂时只运行 bowl 和 box
METHODS=(naive_unlearning)
...
OUTPUT_ROOT="${EXPERIMENTS_BASE}/cross_dataset_naive_${TIMESTAMP}"
...
echo "=========================================="
echo "跨数据集泛化实验 - Naive Unlearning: bowl + box"
...
echo ""
echo "=========================================="
echo "汇总结果 - Naive Unlearning (Defense=Omni, Attack=GSO)"
echo "=========================================="
